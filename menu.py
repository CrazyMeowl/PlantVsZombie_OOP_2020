...
try:
	...
	class gameover(mainmenu):
		...
		def __init__(self):
			...
			self.state = 'normal'
			self.menu = pygame_menu.Menu(500, 1000, ' ',theme = self.theme)
			self.menu.add_button('Restart the game', self.restart)
			self.menu.add_button('Restart with GodMode', self.godMode)
			self.menu.add_button('Return to main menu', self.callMain)
			self.menu.add_button('Quit',self.quit)
	...
	class winmenu(mainmenu):
		...
		def __init__(self):
			...
			self.font = pygame_menu.font.FONT_NEVIS
			self.theme.widget_font = self.font
			self.image = pygame_menu.baseimage.BaseImage(image_path='Resources/win.png', drawing_mode=pygame_menu.baseimage.IMAGE_MODE_SIMPLE)
			self.theme.background_color = self.image
			self.theme.title_bar_style = pygame_menu.widgets.MENUBAR_STYLE_NONE
			self.state = 'normal'
			self.menu = pygame_menu.Menu(500, 1000, ' ',theme = self.theme)
